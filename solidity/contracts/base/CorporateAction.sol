        ...
        if (index < vP.processedShareholders) { //if the shareholder has already been processed
            unchecked {
                ...
                if (transferredUnprocessed > 0) {
                    transferCorporateActionExecution(to, transferredUnprocessed, vP, id, processed);
                }
            }
        }
    }

    function transferCorporateActionExecution(address to, uint256 transferredUnprocessed, VoteParameters storage vP, uint256 id, mapping(address => uint256) storage processed) private {
